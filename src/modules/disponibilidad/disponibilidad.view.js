  ...
  form.addEventListener('submit', async (e) => {
    ...
    const candidatas = unidades
      ...
      .map((u) => {
        ...
        return { unidad: u, rs, libre: estaLibre(rs, entrada, salida) };
      })
      .filter((x) => x.libre)
    ...
    const seccion = el('div', {}, [el('h3', {}, `${candidatas.length} disponible(s) · ${n} noche(s)`)]);

    candidatas.forEach(({ unidad, rs, encaje }) => {
